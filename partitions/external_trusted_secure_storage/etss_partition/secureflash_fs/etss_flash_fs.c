...
#define ETSS_FLASH_FS_FLAG_DELETE          (1U << 24)

static psa_status_t etss_flash_fs_delete_idx(struct etss_flash_fs_ctx_t *fs_ctx,
                                             uint32_t del_file_idx);

static psa_status_t etss_flash_fs_file_write_aligned_data(
                                     ...
                                     const uint8_t *data)
{
#if (ETSS_FLASH_MAX_ALIGNMENT != 1)
    /* Check that the offset is aligned with the flash program unit */
    if (!ETSS_UTILS_IS_ALIGNED(offset, fs_ctx->cfg->program_unit)) {
        return PSA_ERROR_INVALID_ARGUMENT;
    }

    /* Set the size to be aligned with the flash program unit */
    size = ETSS_UTILS_ALIGN(size, fs_ctx->cfg->program_unit);
#endif
    ...
    if (etss_utils_check_contained_in(file_meta->max_size, offset, size)
        != PSA_SUCCESS) {
        return PSA_ERROR_INVALID_ARGUMENT;
    }

    return etss_flash_fs_dblock_write_file(fs_ctx, block_meta, file_meta,
                                           offset, size, data);
}

/* TODO This is very similar to (static) etss_num_active_dblocks() */
static uint32_t etss_flash_fs_num_active_dblocks(
...
static size_t etss_flash_fs_all_metadata_size(
                                       const struct etss_flash_fs_config_t *cfg)
{
    return sizeof(struct etss_metadata_block_header_t)
           + (etss_flash_fs_num_active_dblocks(cfg)
              * sizeof(struct etss_block_meta_t))
           + (cfg->max_num_files * sizeof(struct etss_file_meta_t));
}
 ...
 */
static psa_status_t etss_flash_fs_validate_config(
                                       const struct etss_flash_fs_config_t *cfg)
{
    ...
    if (cfg->num_blocks == 2) {
        ...
        if (cfg->max_file_size >
                       cfg->block_size - etss_flash_fs_all_metadata_size(cfg)) {
            ret = PSA_ERROR_INVALID_ARGUMENT;
        }
    }
    /* It is not required that all files fit in ETSS flash area at the same time.
     * So, it is possible that a create action fails because flash is full.
     * However, the larger file must have enough space in the ETSS flash area to
     * be created, at least, when the ETSS flash area is empty.
     */
    ...
    if (etss_flash_fs_all_metadata_size(cfg) > cfg->block_size) {
        ret = PSA_ERROR_INVALID_ARGUMENT;
    }
...
psa_status_t etss_flash_fs_init_ctx(etss_flash_fs_ctx_t *fs_ctx,
                                    ...
                                    const struct etss_flash_fs_ops_t *fs_ops)
{
    ...
    err = etss_flash_fs_validate_config(fs_cfg);
    if (err != PSA_SUCCESS) {
        return err;
    }
...
psa_status_t etss_flash_fs_prepare(etss_flash_fs_ctx_t *fs_ctx)
{
    psa_status_t err;
    uint32_t idx;

    /* Initialize metadata block with the valid/active metablock */
    err = etss_flash_fs_mblock_init(fs_ctx);
    if (err != PSA_SUCCESS) {
        return err;
    }
     ...
     */
    err = etss_flash_fs_mblock_get_file_idx_flag(fs_ctx,
                                                 ETSS_FLASH_FS_FLAG_DELETE,
                                                 &idx);
    if (err == PSA_SUCCESS) {
        return etss_flash_fs_delete_idx(fs_ctx, idx);
...
psa_status_t etss_flash_fs_wipe_all(struct etss_flash_fs_ctx_t *fs_ctx)
{
    /* Clean and initialize the metadata block */
    return etss_flash_fs_mblock_reset_metablock(fs_ctx);
}

psa_status_t etss_flash_fs_file_exist(struct etss_flash_fs_ctx_t *fs_ctx,
                                      const uint8_t *fid)
{
    psa_status_t err;
    uint32_t idx;

    err = etss_flash_fs_mblock_get_file_idx(fs_ctx, fid, &idx);
    if (err != PSA_SUCCESS) {
...
psa_status_t etss_flash_fs_file_get_info(struct etss_flash_fs_ctx_t *fs_ctx,
                                         const uint8_t *fid,
                                         struct etss_file_info_t *info)
{
    psa_status_t err;
    uint32_t idx;
    ...
    /* Read file metadata */
    err = etss_flash_fs_mblock_read_file_meta(fs_ctx, idx, &tmp_metadata);
    if (err != PSA_SUCCESS) {
        return err;
    }
    ...
    info->flags = tmp_metadata.flags & ETSS_FLASH_FS_USER_FLAGS_MASK;

    return PSA_SUCCESS;
}

psa_status_t etss_flash_fs_file_write(struct etss_flash_fs_ctx_t *fs_ctx,
                                      const uint8_t *fid,
                                      uint32_t flags,
                                      size_t max_size,
                                      size_t data_size,
                                      ...
                                      const uint8_t *data)
{
    ...
    struct etss_file_meta_t file_meta = {0};
    uint32_t cur_phys_block;
    psa_status_t err;
    uint32_t idx;
    uint32_t old_idx = ETSS_METADATA_INVALID_INDEX;
    uint32_t new_idx = ETSS_METADATA_INVALID_INDEX;
    bool use_spare;

    /* Do not permit the user to pass filesystem-internal flags */
    if (flags & ETSS_FLASH_FS_INTERNAL_FLAGS_MASK) {
        return PSA_ERROR_INVALID_ARGUMENT;
    }

#if (ETSS_FLASH_MAX_ALIGNMENT != 1)
    /* Set the max_size to be aligned with the flash program unit */
    max_size = ETSS_UTILS_ALIGN(max_size, fs_ctx->cfg->program_unit);
#endif

    /* Check if the file already exists */
    err = etss_flash_fs_mblock_get_file_idx(fs_ctx, fid, &old_idx);
    if (err == PSA_SUCCESS) {
        /* Read existing file metadata */
        err = etss_flash_fs_mblock_read_file_meta(fs_ctx, old_idx, &file_meta);
        if (err != PSA_SUCCESS) {
            return PSA_ERROR_DOES_NOT_EXIST;
        }

        if (flags & ETSS_FLASH_FS_FLAG_TRUNCATE) {
            if (file_meta.max_size == max_size) {
                 ...
                 */
                file_meta.flags |= ETSS_FLASH_FS_FLAG_DELETE;
                err = etss_flash_fs_mblock_update_scratch_file_meta(fs_ctx,
                                                                    old_idx,
                                                                    &file_meta);
                if (err != PSA_SUCCESS) {
        ...
        if (!(flags & ETSS_FLASH_FS_FLAG_CREATE)) {
            return PSA_ERROR_DOES_NOT_EXIST;
        }
    } else {
        return err;
    ...
    if (new_idx == ETSS_METADATA_INVALID_INDEX) {
        ...
        use_spare = (old_idx != ETSS_METADATA_INVALID_INDEX);

        /* Try to reserve a new file based on the input parameters */
        err = etss_flash_fs_mblock_reserve_file(fs_ctx, fid, use_spare,
    ...
    if (data_size != 0) {
        ...
        err = etss_flash_fs_file_write_aligned_data(fs_ctx, &block_meta,
                                                    &file_meta, offset,
                                                    data_size, data);
        if (err != PSA_SUCCESS) {
            return PSA_ERROR_GENERIC_ERROR;
        ...
        block_meta.phy_id =
            etss_flash_fs_mblock_cur_data_scratch_id(fs_ctx, file_meta.lblock);

        /* Swap the scratch data block */
        etss_flash_fs_mblock_set_data_scratch(fs_ctx, cur_phys_block,
                                              file_meta.lblock);
    }

    /* Update block metadata in scratch metadata block */
    err = etss_flash_fs_mblock_update_scratch_block_meta(fs_ctx,
                                                         file_meta.lblock,
                                                         &block_meta);
    if (err != PSA_SUCCESS) {
        return PSA_ERROR_GENERIC_ERROR;
    ...
    err = etss_flash_fs_mblock_update_scratch_file_meta(fs_ctx, new_idx,
                                                        &file_meta);
    if (err != PSA_SUCCESS) {
        return PSA_ERROR_GENERIC_ERROR;
    }
     ...
     * indexes.
     */
    idx = ETSS_UTILS_MIN(new_idx, old_idx);
    err = etss_flash_fs_mblock_cp_file_meta(fs_ctx, 0, idx);
    if (err != PSA_SUCCESS) {
        return PSA_ERROR_GENERIC_ERROR;
    }

    /* Copy the file metadata entries between the two indexes, if necessary */
    if (old_idx != ETSS_METADATA_INVALID_INDEX && old_idx != new_idx) {
        err = etss_flash_fs_mblock_cp_file_meta(fs_ctx, idx + 1,
                                              ETSS_UTILS_MAX(new_idx, old_idx));
        if (err != PSA_SUCCESS) {
            return PSA_ERROR_GENERIC_ERROR;
        }

        idx = ETSS_UTILS_MAX(new_idx, old_idx);
    }
     ...
     * the file data.
     */
    if ((file_meta.lblock != ETSS_LOGICAL_DBLOCK0) || (data_size == 0)) {
        err = etss_flash_fs_mblock_migrate_lb0_data_to_scratch(fs_ctx);
        if (err != PSA_SUCCESS) {
            return PSA_ERROR_GENERIC_ERROR;
        }
    }
    ...
    err = etss_flash_fs_mblock_meta_update_finalize(fs_ctx);
    if (err != PSA_SUCCESS) {
        return err;
    }
     ...
     */
    if (old_idx != ETSS_METADATA_INVALID_INDEX && old_idx != new_idx) {
        err = etss_flash_fs_delete_idx(fs_ctx, old_idx);
    }
                                             ...
                                             uint32_t del_file_idx)
{
    ...
    err = etss_flash_fs_mblock_read_file_meta(fs_ctx, del_file_idx, &file_meta);
    if (err != PSA_SUCCESS) {
        return err;
    }
    ...
    file_meta = (struct etss_file_meta_t){0};

    /* Update file metadata in to the scratch block */
    err = etss_flash_fs_mblock_update_scratch_file_meta(fs_ctx, del_file_idx,
                                                        &file_meta);
    if (err != PSA_SUCCESS) {
        return err;
    }
    ...
    for (idx = 0; idx < fs_ctx->cfg->max_num_files; idx++) {
        ...
        err = etss_flash_fs_mblock_read_file_meta(fs_ctx, idx, &file_meta);
        if (err != PSA_SUCCESS) {
            return err;
        }
        ...
        if ((file_meta.lblock == del_file_lblock) &&
            (etss_utils_validate_fid(file_meta.id) == PSA_SUCCESS)) {
        ...
        /* Update file metadata in to the scratch block */
        err = etss_flash_fs_mblock_update_scratch_file_meta(fs_ctx, idx,
                                                            &file_meta);
        if (err != PSA_SUCCESS) {
            return err;
        }
    ...
    /* Compact data block */
    err = etss_flash_fs_dblock_compact_block(fs_ctx, del_file_lblock,
                                             del_file_max_size,
     ...
     */
    if (del_file_lblock != ETSS_LOGICAL_DBLOCK0) {
        err = etss_flash_fs_mblock_migrate_lb0_data_to_scratch(fs_ctx);
        if (err != PSA_SUCCESS) {
            return PSA_ERROR_GENERIC_ERROR;
        }
...
psa_status_t etss_flash_fs_file_delete(struct etss_flash_fs_ctx_t *fs_ctx,
                                       const uint8_t *fid)
{
    psa_status_t err;
    uint32_t del_file_idx;
    ...
    /* Get the file index */
    err = etss_flash_fs_mblock_get_file_idx(fs_ctx, fid, &del_file_idx);
    if (err != PSA_SUCCESS) {
        return PSA_ERROR_DOES_NOT_EXIST;
    }

    return etss_flash_fs_delete_idx(fs_ctx, del_file_idx);
}

psa_status_t etss_flash_fs_file_read(struct etss_flash_fs_ctx_t *fs_ctx,
                                     const uint8_t *fid,
                                     size_t size,
                                     size_t offset,
                                     uint8_t *data)
{
    ...
    err = etss_utils_check_contained_in(tmp_metadata.cur_size, offset, size);
    if (err != PSA_SUCCESS) {
        return err;
    }

    /* Read the file from flash */
    err = etss_flash_fs_dblock_read_file(fs_ctx, &tmp_metadata, offset, size,
                                         data);
    if (err != PSA_SUCCESS) {
        return PSA_ERROR_GENERIC_ERROR;
    }
