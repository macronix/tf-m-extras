...
#ifndef __ETSS_FLASH_FS_H__
#define __ETSS_FLASH_FS_H__
 ...
 */
#define ETSS_FLASH_FS_USER_FLAGS_MASK   ((1U << 16) - 1)

/* Filesystem flags that can be provided when calling the write function */
#define ETSS_FLASH_FS_WRITE_FLAGS_MASK  ((1U << 24) - (1U << 16))
/* Create the file if it does not already exist */
#define ETSS_FLASH_FS_FLAG_CREATE       (1U << 16)
/* Remove existing file data if it exists */
#define ETSS_FLASH_FS_FLAG_TRUNCATE     (1U << 17)

/* Invalid block index */
#define ETSS_BLOCK_INVALID_ID 0xFFFFFFFFU
...
struct etss_flash_fs_ops_t {
     ...
     */
    psa_status_t (*init)(const struct etss_flash_fs_config_t *cfg);
     ...
     *         Otherwise, it returns PSA_ERROR_STORAGE_FAILURE.
     */
    psa_status_t (*read)(const struct etss_flash_fs_config_t *cfg,
     ...
     *         Otherwise, it returns PSA_ERROR_STORAGE_FAILURE.
     */
    psa_status_t (*write)(const struct etss_flash_fs_config_t *cfg,
     ...
     *        etss_flash_block_to_block_move()) for one block ID, before any call
     ...
     *         Otherwise, it returns PSA_ERROR_STORAGE_FAILURE.
     */
    psa_status_t (*flush)(const struct etss_flash_fs_config_t *cfg,
                          uint32_t block_id);
     ...
     *         Otherwise, it returns PSA_ERROR_STORAGE_FAILURE.
     */
    psa_status_t (*erase)(const struct etss_flash_fs_config_t *cfg,
                          uint32_t block_id);
};

/**
 * \brief etss flash filesystem context type, used to maintain state across FS
 *        operations.
 *
 * \details The user should allocate a variable of this type, initialised to
 *          zero, before calling etss_flash_fs_prepare, and then pass it to each
 *          subsequent FS operation. The contents are internal to the
 *          filesystem.
 */
typedef struct etss_flash_fs_ctx_t etss_flash_fs_ctx_t;

/*!
 * \struct etss_file_info_t
 *
 ...
 *                        initialised by etss_flash_fs_init_ctx().
 *
 * \return Returns error code as specified in \ref psa_status_t
 */
 ...
 * \return Returns error code as specified in \ref psa_status_t
 */
psa_status_t etss_flash_fs_wipe_all(etss_flash_fs_ctx_t *fs_ctx);
 ...
 */
psa_status_t etss_flash_fs_file_exist(etss_flash_fs_ctx_t *fs_ctx,
                                      const uint8_t *fid);
 ...
 *                        file information values \ref etss_file_info_t
 *
 * \return Returns error code specified in \ref psa_status_t
 */
psa_status_t etss_flash_fs_file_get_info(etss_flash_fs_ctx_t *fs_ctx,
                                         const uint8_t *fid,
 ...
 * \return Returns error code as specified in \ref psa_status_t
 */
psa_status_t etss_flash_fs_file_write(etss_flash_fs_ctx_t *fs_ctx,
                                      const uint8_t *fid,
                                      uint32_t flags,
                                      size_t max_size,
                                      size_t data_size,
 ...
 * \return Returns error code as specified in \ref psa_status_t
 */
psa_status_t etss_flash_fs_file_read(etss_flash_fs_ctx_t *fs_ctx,
                                     const uint8_t *fid,
                                     size_t size,
                                     size_t offset,
                                     uint8_t *data);
 ...
 * \return Returns error code as specified in \ref psa_status_t
 */
psa_status_t etss_flash_fs_file_delete(etss_flash_fs_ctx_t *fs_ctx,
                                       const uint8_t *fid);
