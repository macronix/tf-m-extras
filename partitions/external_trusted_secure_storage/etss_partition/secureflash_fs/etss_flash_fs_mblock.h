...
#ifndef __ETSS_FLASH_FS_MBLOCK_H__
#define __ETSS_FLASH_FS_MBLOCK_H__
...
/*!
 * \def ETSS_SUPPORTED_VERSION
 *
 * \brief Defines the supported version.
 */
#define ETSS_SUPPORTED_VERSION  0x02

/*!
 * \def ETSS_BACKWARD_SUPPORTED_VERSION
 *
 * \brief Defines the backward supported version.
 */
#define ETSS_BACKWARD_SUPPORTED_VERSION  0x01

/*!
 * \def ETSS_METADATA_INVALID_INDEX
 *
 * \brief Defines the invalid index value when the metadata table is full
 */
#define ETSS_METADATA_INVALID_INDEX 0xFFFF

/*!
 * \def ETSS_LOGICAL_DBLOCK0
 *
 * \brief Defines logical data block 0 ID
 */
#define ETSS_LOGICAL_DBLOCK0  0

/*!
 * \struct etss_metadata_block_header_t
 *
 ...
 *       etss_metadata_block_header_comp_t.
 *
...
struct etss_metadata_block_header_t {
    _T1
#if ((ETSS_FLASH_MAX_ALIGNMENT) > 4)
    uint8_t roundup[sizeof(struct __attribute__((__aligned__(ETSS_FLASH_MAX_ALIGNMENT))) { _T1 }) -
...
/*!
 * \struct etss_metadata_block_header_comp_t
 *
...
struct etss_metadata_block_header_comp_t {
    _T1_COMP
#if ((ETSS_FLASH_MAX_ALIGNMENT) > 4)
    uint8_t roundup[sizeof(struct __attribute__((__aligned__(ETSS_FLASH_MAX_ALIGNMENT))) { _T1_COMP }) -
...
struct etss_block_meta_t {
    _T2
#if ((ETSS_FLASH_MAX_ALIGNMENT) > 4)
    uint8_t roundup[sizeof(struct __attribute__((__aligned__(ETSS_FLASH_MAX_ALIGNMENT))) { _T2 }) -
                    sizeof(struct { _T2 })];
#endif
};
#undef _T2
...
#define _T3 \
    ...
    uint8_t id[ETSS_FILE_ID_SIZE];  /*!< ID of this file */

struct etss_file_meta_t {
    _T3
#if ((ETSS_FLASH_MAX_ALIGNMENT) > 4)
    uint8_t roundup[sizeof(struct __attribute__((__aligned__(ETSS_FLASH_MAX_ALIGNMENT))) { _T3 }) -
                    sizeof(struct { _T3 })];
#endif
};
#undef _T3
...
/**
 * \struct etss_flash_fs_ctx_t
 *
 * \brief Structure to store the ETSS flash file system context.
 */
struct etss_flash_fs_ctx_t {
    const struct etss_flash_fs_config_t *cfg; /**< Filesystem configuration */
    const struct etss_flash_fs_ops_t *ops;    /**< Filesystem flash operations */
    struct etss_metadata_block_header_t meta_block_header; /**< Metadata block
                                                           *   header
                                                           */
...
#endif /* __ETSS_FLASH_FS_MBLOCK_H__ */
