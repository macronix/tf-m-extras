 ...
 */
static uint32_t etss_dblock_lo_to_phy(struct etss_flash_fs_ctx_t *fs_ctx,
                                      uint32_t lblock)
{
    struct etss_block_meta_t block_meta;
    psa_status_t err;

    err = etss_flash_fs_mblock_read_block_metadata(fs_ctx, lblock, &block_meta);
    if (err != PSA_SUCCESS) {
        return ETSS_BLOCK_INVALID_ID;
    }

    return block_meta.phy_id;
}

psa_status_t etss_flash_fs_dblock_compact_block(
                                             ...
                                             size_t size)
{
    ...
    scratch_id = etss_flash_fs_mblock_cur_data_scratch_id(fs_ctx, lblock);
    ...
    if (size > 0) {
         ...
         * destination offset.
         */
        err = etss_flash_fs_block_to_block_move(fs_ctx, scratch_id, dst_offset,
    ...
    if (dst_offset > block_meta.data_start) {
         ...
         */
        err = etss_flash_fs_block_to_block_move(
                                              fs_ctx, scratch_id,
     ...
     * of finalization.
     */
    etss_flash_fs_mblock_set_data_scratch(fs_ctx, block_meta.phy_id, lblock);
    ...
    err = etss_flash_fs_mblock_update_scratch_block_meta(fs_ctx, lblock,
                                                         &block_meta);
    if (err != PSA_SUCCESS) {
        /* Swap back the data block as there was an issue in the process */
        etss_flash_fs_mblock_set_data_scratch(fs_ctx, scratch_id, lblock);
        return err;
    }
...
psa_status_t etss_flash_fs_dblock_read_file(
                                       struct etss_flash_fs_ctx_t *fs_ctx,
                                       ...
                                       uint8_t *buf)
{
    ...
    phys_block = etss_dblock_lo_to_phy(fs_ctx, file_meta->lblock);
    if (phys_block == ETSS_BLOCK_INVALID_ID) {
        return PSA_ERROR_GENERIC_ERROR;
    }
...
psa_status_t etss_flash_fs_dblock_write_file(
                                     struct etss_flash_fs_ctx_t *fs_ctx,
                                     ...
                                     const uint8_t *data)
{
    ...
    scratch_id = etss_flash_fs_mblock_cur_data_scratch_id(fs_ctx,
                                                          file_meta->lblock);
    ...
    err = etss_flash_fs_block_to_block_move(fs_ctx, scratch_id,
                                            block_meta->data_start,
                                            block_meta->phy_id,
                                            block_meta->data_start,
    ...
    err = etss_flash_fs_block_to_block_move(fs_ctx, scratch_id, pos,
