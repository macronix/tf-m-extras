...
int32_t spi_nor_reset_enable(void)
{
    ...
    return spi_write(spi_nor.cmd_packet.cmd_buffer.tx_buf, spi_nor.cmd_packet.cmd_buffer.tx_len);
}
...
int32_t spi_nor_read_id(uint8_t *id, uint32_t size)
{
    ...
    spi_nor.cmd_packet.data = NULL;
    cmd_packing(&spi_nor.cmd_packet);
    return spi_read(spi_nor.cmd_packet.cmd_buffer.tx_buf,
...
int32_t spi_nor_init(void)
{
    int32_t status;
    status = spi_nor_hc_init();
    if (status) {
        goto exit_point_init;
    }
    if (spi_nor_software_reset() != 0) {
        goto exit_point_init;
    }
    return 0;
exit_point_init:
