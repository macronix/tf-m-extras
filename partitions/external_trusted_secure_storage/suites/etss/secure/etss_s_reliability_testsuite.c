...
/* Define test suite for ETSS reliability tests */
/* List of tests */
static void tfm_etss_test_3001(struct test_result_t *ret);
static void tfm_etss_test_3002(struct test_result_t *ret);

static struct test_t reliability_tests[] = {
    {&tfm_etss_test_3001, "TFM_ETSS_TEST_3001",
     "repetitive sets and gets in/from an asset", {TEST_PASSED} },
    {&tfm_etss_test_3002, "TFM_ETSS_TEST_3002",
...
void register_testsuite_s_psa_etss_reliability(struct test_suite_t *p_test_suite)
{
    ...
    set_testsuite("ETSS reliability tests (TFM_ETSS_TEST_3XXX)",
...
static void tfm_etss_test_3001(struct test_result_t *ret)
{
    ...
    for (itr = 0; itr < LOOP_ITERATIONS_001; itr++) {
        ...
        err = tfm_etss_get(uid, offset, data_len, (read_data +
                                                    HALF_PADDING_SIZE),
                                                    &read_data_len);
        if (err != ETSS_SUCCESS) {
